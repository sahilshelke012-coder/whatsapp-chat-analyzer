...
def compute_chat_analytics(messages):
    """
    Computes complete analytics from parsed message objects.
    """
    # Filter out System messages if desired or include as system
    user_messages = [m for m in messages if m['author'] != 'System']
    
    if not user_messages:
        # Default empty response structure if file has no user messages
        return {
            'overall': {
                'totalMessages': 0,
                'totalWords': 0,
    ...
    day_of_week_counts = Counter()  # Mon, Tue, etc.
    ...
    for m in user_messages:
        ...
        if is_media_message(msg_text):
            ...
            # Extract clean words for content analysis (excluding stopwords & numbers)
            for w in words:
                clean_w = re.sub(r'[^a-zA-Z0-9\u0900-\u097F]', '', w)  # Support English & Hindi/regional unicode
    ...
    # Sort day of week naturally (Monday -> Sunday)
    ...
    return {
        ...
        'contentAnalysis': {
            ...
            'sharedLinks': shared_links[:50],  # Return top 50 shared links
            'mediaStats': {
                'totalMedia': total_media,
